...
export default function RecallPage() {
  ...
  const filteredLots = useMemo(() => {
    ...
    if (!q) return lots;
    return lots.filter((l) => l.lot_code.toLowerCase().includes(q) || l.item_name.toLowerCase().includes(q));
  }, [lots, search]);
  ...
  const backwardLots = useMemo(() => {
    ...
    return recall.backward_lot_ids
      .map((id) => lotById.get(id) || { id, lot_code: `#${id}`, state: "?", item_id: 0, item_name: "Unknown", received_at: "" })
  ...
  async function quarantineForward() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8000"}/recall/${selectedLotId}/quarantine-forward`, {
        method: "POST",
    ...
    <main className="min-h-screen p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Select a lot and trace backward (sources) + forward (derived lots) + affected customers.
        </p>
          ...
          {selectedLot && (
            <div className="rounded-xl border p-3 bg-gray-50 text-sm">
              ...
              <div className="mt-1">
                ...
                <span className="font-medium">{selectedLot.state}</span>
              </div>
              <div className="text-gray-600">lot_id: {selectedLot.id}</div>
            </div>
          )}
        ...
        {recall && (
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="rounded-xl border p-4">
              ...
              <div className="text-sm text-gray-600 mt-1">Source lots (inputs)</div>
              <div className="mt-3 space-y-2">
                {backwardLots.length === 0 ? (
                  <div className="text-sm text-gray-600">No ancestors found.</div>
                ) : (
                  backwardLots.map((l) => (
                    <div key={l.id} className="rounded-lg border p-2">
                      <div className="font-mono text-sm">{l.lot_code}</div>
                      <div className="text-xs text-gray-600">{l.item_name} — {l.state} — id {l.id}</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="rounded-xl border p-4">
              ...
              <div className="text-sm text-gray-600 mt-1">Derived lots (outputs)</div>
              <div className="mt-3 space-y-2">
                {forwardLots.length === 0 ? (
                  <div className="text-sm text-gray-600">No descendants found.</div>
                ) : (
                  forwardLots.map((l) => (
                    <div key={l.id} className="rounded-lg border p-2">
                      <div className="font-mono text-sm">{l.lot_code}</div>
            ...
            <div className="rounded-xl border p-4">
              ...
              <div className="text-sm text-gray-600 mt-1">Customers who bought this lot or its descendants</div>
              <div className="mt-3 space-y-2">
                {recall.affected_customers.length === 0 ? (
                  ...
                  recall.affected_customers.map((c) => (
                    <div key={c.id} className="rounded-lg border p-2">
                      ...
                      <div className="text-xs text-gray-600">customer_id: {c.id}</div>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="rounded-xl border p-4 mt-4">
            ...
            <div className="text-sm text-gray-600 mt-1">
              Quarantines all <span className="font-medium">forward (descendant)</span> lots found in the trace.
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
              <div>
                <label className="text-sm font-medium">Performed By (user id)</label>
                <input className="mt-1 w-full border rounded-lg p-2" value={performedBy} onChange={(e) => setPerformedBy(e.target.value)} inputMode="numeric" />
              </div>
              <div className="sm:col-span-2">
                <label className="text-sm font-medium">Reason</label>
                <input className="mt-1 w-full border rounded-lg p-2" value={reason} onChange={(e) => setReason(e.target.value)} />
              </div>
            </div>

            <button
              className="mt-3 w-full rounded-lg bg-red-600 text-white p-2 disabled:opacity-50"
              disabled={busy || !recall || recall.forward_lot_ids.length === 0}
              onClick={quarantineForward}
            >
              {busy ? "Quarantining…" : `Quarantine all forward lots (${recall.forward_lot_ids.length})`}
            </button>

            {recall.forward_lot_ids.length === 0 && (
              <div className="text-sm text-gray-600 mt-2">No forward lots found — nothing to quarantine.</div>
            )}

            {quarantineResult && (
              <div className="mt-3 rounded-xl border border-green-300 bg-green-50 p-3 text-sm text-green-900">
                <div className="font-medium">Quarantine complete</div>
                <div>Quarantined: {quarantineResult.quarantined_count}</div>
                <div>Already quarantined: {quarantineResult.already_quarantined_count}</div>
              </div>
            )}
          </div>
        )}
